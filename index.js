...
// ── DB Helpers ────────────────────────────────────────────────────────────────
const getUser = (id) => db.prepare('SELECT * FROM users WHERE telegram_id = ?').get(String(id));
const getUserByUsername = (u) => db.prepare('SELECT * FROM users WHERE telegram_username = ?').get(u);
const getUserByPhone = (p) => db.prepare('SELECT * FROM users WHERE phone_number = ?').get(p);
const upsertUser = (id, username, name) => {
  db.prepare(`INSERT INTO users (telegram_id, telegram_username, full_name) VALUES (?,?,?)
    ...
    full_name=excluded.full_name`).run(String(id), username||null, name||null);
  return getUser(id);
};
const setPhone = (id, phone) => db.prepare('UPDATE users SET phone_number=? WHERE telegram_id=?').run(phone, String(id));
const getSession = (id) => {
  const r = db.prepare('SELECT * FROM sessions WHERE telegram_id=?').get(String(id));
  return r ? { step: r.step, data: JSON.parse(r.data||'{}') } : null;
};
const setSession = (id, step, data={}) => db.prepare(`INSERT INTO sessions (telegram_id,step,data) VALUES (?,?,?)
  ON CONFLICT(telegram_id) DO UPDATE SET step=excluded.step, data=excluded.data`)
  .run(String(id), step, JSON.stringify(data));
const clearSession = (id) => db.prepare('DELETE FROM sessions WHERE telegram_id=?').run(String(id));
const saveTx = (f) => db.prepare(`INSERT OR IGNORE INTO transactions
  (reference_id,type,status,sender_id,recipient_phone,amount,description)
  VALUES (?,?,?,?,?,?,?)`).run(f.ref, f.type, f.status||'pending', f.senderId||null, f.phone, f.amount, f.desc||null);
const updateTx = (ref, status) => db.prepare('UPDATE transactions SET status=? WHERE reference_id=?').run(status, ref);
const getTxHistory = (id) => db.prepare(`SELECT * FROM transactions WHERE sender_id=?
  ORDER BY created_at DESC LIMIT 10`).all(String(id));
const getChannel = (id) => db.prepare('SELECT * FROM channels WHERE channel_id=?').get(String(id));
const saveChannel = (id, name, adminId, phone) => db.prepare(`INSERT INTO channels
  (channel_id,channel_name,admin_id,admin_phone) VALUES (?,?,?,?)
  ON CONFLICT(channel_id) DO UPDATE SET channel_name=excluded.channel_name,
  admin_phone=excluded.admin_phone`).run(String(id), name, String(adminId), phone);
...
function normalizePhone(p) { return p.replace(/^\+/,'').replace(/^0/,'256'); }
...
async function requestToPay(amount, phone, desc, ref) {
  ...
  }, { headers: { Authorization: `Bearer ${token}`, 'X-Reference-Id': ref,
    'X-Target-Environment': ENV, 'Ocp-Apim-Subscription-Key': process.env.MOMO_COLLECTIONS_PRIMARY_KEY,
...
async function checkPayment(ref) {
  ...
  const res = await axios.get(`${BASE}/collection/v1_0/requesttopay/${ref}`, {
    headers: { Authorization: `Bearer ${token}`, 'X-Target-Environment': ENV,
...
async function sendMoney(amount, phone, desc, ref) {
  ...
  await axios.post(`${BASE}/disbursement/v1_0/transfer`, {
    ...
    'X-Target-Environment': ENV, 'Ocp-Apim-Subscription-Key': process.env.MOMO_DISBURSEMENTS_PRIMARY_KEY,
    'Content-Type': 'application/json' } });
}
...
bot.onText(/\/start(?:\s+(.+))?/, async (msg, match) => {
  ...
  // Handle channel payment deep link
  ...
  await bot.sendMessage(chatId,
    ...
    (user?.phone_number ? `📱 Linked: \`${user.phone_number}\`\n\n` : `⚠️ No number linked yet. Use /register\n\n`) +
    `*Commands:*\n/register — link MoMo number\n/pay — send money\n/request — request payment\n` +
...
bot.onText(/\/history/, (msg) => {
  ...
  const lines = txs.map(t => `${t.status==='successful'?'✅':'❌'} ${fmt(t.amount)} → ${t.recipient_phone}\n   ${t.created_at.split('T')[0]}`).join('\n\n');
...
bot.onText(/\/pay(?:\s+(.+))?/, async (msg, match) => {
  ...
  if (parts.length < 2) return bot.sendMessage(chatId, '❌ Please include target and amount.');
  ...
  const label = recipientUser ? `@${recipientUser.telegram_username||recipientUser.full_name} (${recipientPhone})` : recipientPhone;
...
// ── Message handler (sessions + text input) ───────────────────────────────────
bot.on('message', async (msg) => {
  ...
  // Awaiting phone number for registration
  if (session.step === 'awaiting_phone') {
    ...
    return bot.sendMessage(msg.chat.id, `✅ *Number linked!*\n\`${phone}\` is now linked to your account.`, { parse_mode:'Markdown' });
  }

  // Awaiting channel username for /connect
  if (session.step === 'awaiting_channel') {
    const user = getUser(telegramId);
    const channelUsername = msg.text.startsWith('@') ? msg.text : `@${msg.text}`;
    try {
      const chat = await bot.getChat(channelUsername);
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('payconfirm:')) {
    ...
    try {
      ...
      await bot.sendMessage(chatId, `📲 Check your phone \`${user.phone_number}\` for USSD prompt.\n_Waiting for approval…_`, { parse_mode:'Markdown' });
      const result = await pollStatus(ref);
      if (result.status==='SUCCESSFUL') {
        updateTx(ref,'successful');
  ...
  if (data.startsWith('reqpay:')) {
    ...
    if (!user?.phone_number) return bot.answerCallbackQuery(query.id, { text:'Link your number first /register' });
    ...
    try {
      ...
      if (result.status==='SUCCESSFUL') {
        ...
        bot.sendMessage(requesterId, `💰 *Payment received!* ${fmt(amount)} from ${user.telegram_username?'@'+user.telegram_username:user.full_name}`, { parse_mode:'Markdown' }).catch(()=>{});
      } else {
        bot.sendMessage(chatId, `❌ Payment not completed.`);
      }
    } catch(e) {
  ...
  if (data.startsWith('chanpay:')) {
    ...
    try {
      ...
      if (result.status==='SUCCESSFUL') {
        ...
        bot.sendMessage(channel.admin_id, `💰 New payment! ${fmt(amount)} from ${user.telegram_username?'@'+user.telegram_username:user.full_name}`, { parse_mode:'Markdown' }).catch(()=>{});
      } else {
        updateTx(ref,'failed');
